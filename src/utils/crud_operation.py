...
class Data_base_operation():
    ...
    async def create_short_url(db:Session, original_url:str) -> None:
        ...
        #existing_url = await db.query(Url).filter(Url.original_url==original_url).first()
        existing_url = await db.execute(select(Url).where(Url.original_url==original_url))
        ...
        while not (existing_short_url==None):
            ...
            #existing_short_url = await db.query(Url).filter(Url.short_url==short_url).first()
            existing_short_url = await db.execute(select(Url).where(Url.short_url==short_url))
    ...
    async def get_original_url(db:Session, short_url:str):
        ...
        #original_url = await db.query(Url).filter(Url.short_url==short_url).first()
        original_url = await db.execute(select(Url).where(Url.short_url==short_url))
