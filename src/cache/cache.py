...
class CacheMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request, call_next):
        ...
        if not self.cache_enabled : # or request.method != "GET"
            # Passer directement à la requête suivante si le cache est désactivé ou si la méthode n'est pas GET
        ...
        if isinstance(response, (StreamingResponse,)) or type(response).__name__ == "_StreamingResponse":            
            content = b""
